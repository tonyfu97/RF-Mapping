"""
Script to calculate the intersection over union (IOU) between the ground
truth and the bar maps. This two maps are first thresholded to 50% max to
produce two binary images.
...
def compute_iou(map1, map2):
    # If the map has color, remove the color channel.
    if len(map1.shape) == 3:
        map1 = map1.max(axis=2)
    if len(map2.shape) == 3:
        map2 = map2.max(axis=2)
...
txt_path = os.path.join(result_dir, f"{max_or_min}_map_iou_{sigma_rf_ratio:.4f}.txt")
        ...
        for idx1, (name1, map1) in enumerate(all_smoothed_maps.items()):
            for idx2, (name2, map2) in enumerate(all_smoothed_maps.items()):
                if idx1 <= idx2:
                    ...
                    r_val = compute_iou(unit_map1, unit_map2)
                    r_vals.append(r_val)
...
if to_plot_pdf:
    for conv_i in range(num_layers):
        ...
        pdf_path = os.path.join(result_dir, f"{layer_name}_{max_or_min}_map_iou_{sigma_rf_ratio:.4f}.pdf")
        with PdfPages(pdf_path) as pdf:
            ...
            for unit_i in tqdm(range(num_units)):
                ...
                plt.suptitle(f"IOU of different maps (no.{unit_i}, {max_or_min})", fontsize=32)
            ...
            plt.title(f"Distribution of IOU higher than {r_val_threshold}", fontsize=font_size)
            pdf.savefig()
            plt.show()
            plt.close()
