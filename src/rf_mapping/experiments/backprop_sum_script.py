...
for conv_i, rf_size in enumerate(rf_sizes):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for unit_i in tqdm(range(num_units)):
            max_sum = np.zeros((rf_size[0], rf_size[1], 3))
            min_sum = np.zeros((rf_size[0], rf_size[1], 3))
            ...
            for img_i in range(num_images):
                ...
                max_sum = add_patch_to_sum(max_grad_patch_padded, max_sum, sum_mode)
                ...
                min_sum = add_patch_to_sum(min_grad_patch_padded, min_sum, sum_mode)
            ...
            plt.subplot(1, 3, 1)
            plt.imshow(preprocess_img_for_plot(max_sum_norm))
            plt.title("max", fontsize=20)
            plt.subplot(1, 3, 2)
            plt.imshow(preprocess_img_for_plot(min_sum_norm))
            plt.title("min", fontsize=20)
            plt.subplot(1, 3, 3)
            plt.imshow(preprocess_img_for_plot(both_sum_norm))
            ...
            both_result_path = os.path.join(result_dir, f"both_conv{conv_i+1}.{unit_i}.npy")
            np.save(max_result_path, max_sum_norm)
            np.save(min_result_path, min_sum_norm)
            np.save(both_result_path, both_sum_norm)
