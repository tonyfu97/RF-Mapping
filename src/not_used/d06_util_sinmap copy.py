...
#                                                                             #
###############################################################################
def rfmap_top_sin1_old(m,xn,max_rf,netid,layname,showflag):
  #
  #  m        - model to run
...
def rfmap_top_sin1(m,xn,max_rf,netid,layname,showflag):
  ...
  while i0 < n:
    ...
    for i in range(ns):
      s = stimset_stim_get_sin(splist[i0+i])
      d[i][0] = s
      d[i][1] = s
      d[i][2] = s
    i0 += ns
  ...
  print("  Length of stimulus parameter list:",len(splist))
  print("  Length of model response list:  ",len(srlist[0]))
  # Note, there are 113,280 stimuli
  
  statfile = netid + '_stat_' + layname + '_mrf_sin1_NEW.txt'
  pdffile = netid + '_' + layname + '_sin1_NEW.pdf'
